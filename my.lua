...
Sections.MainFeatures:AddToggle("DoubleDamage_Toggle", {
    ...
    Callback = function(isEnabled)
        ...
        if isEnabled then
            toggleDoubleDamage(true)
        else
            toggleDoubleDamage(false)
        end
    end,
})
...
local function toggleAntiKnockback(enable)
    if enable then
        ...
        if humanoid then
            -- Hook ApplyImpulse to block knockback
            _G.OriginalApplyImpulse = hookfunction(humanoid.ApplyImpulse, function(self, impulse)
                if impulse.Magnitude > 1 then
                    return
                end
                return _G.OriginalApplyImpulse(self, impulse)
            end)

            -- Normalize velocity to prevent residual knockback effects
            game:GetService("RunService").Stepped:Connect(function()
                local rootPart = game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                if rootPart then
                    if rootPart.Velocity.Magnitude > 10 then
        ...
        if _G.OriginalApplyImpulse then
            hookfunction(humanoid.ApplyImpulse, _G.OriginalApplyImpulse)
            _G.OriginalApplyImpulse = nil
        end
    end
end

-- Logic for Double Damage
local function toggleDoubleDamage(enable)
    if enable then
        -- Hook FireServer to modify damage packets
        _G.OriginalFireServer = hookfunction(ByteNetReliable.FireServer, function(self, ...)
            local args = {...}
            if args[1] == buffer.fromstring("\a\001\001") and type(args[2]) == "table" then
                args[2][1] = args[2][1] * 2 -- Double the damage value
            end
            return _G.OriginalFireServer(self, unpack(args))
        end)
    else
        if _G.OriginalFireServer then
            hookfunction(ByteNetReliable.FireServer, _G.OriginalFireServer)
            _G.OriginalFireServer = nil
        end
    end
end
...
local function toggleGodMode(enable)
    ...
    if enable then
        -- Block damage intake
        _G.OriginalTakeDamage = hookfunction(humanoid.TakeDamage, function(self, amount, ...)
            return
        end)
        ...
        -- Prevent negative states (knockout, death)
        _G.OriginalStateChanged = hookfunction(humanoid.StateChanged, function(self, oldState, newState)
            if newState == Enum.HumanoidStateType.Knockout or 
               newState == Enum.HumanoidStateType.Dead then
                return Enum.HumanoidStateType.Landing
            end
            return _G.OriginalStateChanged(self, oldState, newState)
        end)
    else
        if _G.OriginalTakeDamage then
            hookfunction(humanoid.TakeDamage, _G.OriginalTakeDamage)
            _G.OriginalTakeDamage = nil
        end
        if _G.OriginalStateChanged then
            hookfunction(humanoid.StateChanged, _G.OriginalStateChanged)
            _G.OriginalStateChanged = nil
        end
    end
end
