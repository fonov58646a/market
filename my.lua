...
--  4.  Anti-Cheat Bypass & Moderator Detection
local function CheckForModerators()
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            -- Check for moderator roles/attributes
            local isModerator = plr:GetAttribute("IsModerator") or false
            if isModerator then
                return true
            end
        end
...
local function AntiCheatBypass()
    ...
    -- Apply obfuscation to key strings
    local keyStrings = {
        "FireServer",
        "RemoteEvent",
        "RemoteFunction",
    ...
    for _, str in ipairs(keyStrings) do
        getgenv()[obfuscateString(str)] = getgenv()[str]
    end
...
local function AutoKickOnModerator()
    while true do
        if CheckForModerators() then
            game:Shutdown()
        end
...
-- Anti-Cheat Bypass & Moderator Detection
AntiCheatBypass()
task.spawn(AutoKickOnModerator)
